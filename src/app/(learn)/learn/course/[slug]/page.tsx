...
export default async function CourseLandingPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#06060e] text-[#e2e8f0]">
      ...
      {/* ── ENROLL CTA ── */}
      <section className="border-t border-[#1e1e3a] bg-gradient-to-b from-[#0c0c1a] to-[#06060e] px-6 py-24">
        <div className="mx-auto max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="mb-4 text-lg text-[#94a3b8]">
              {course.totalSessions} sessions of hands-on learning for just ₹999.
            </p>
            <p className="mb-6 text-sm text-[#94a3b8]">
              Session 1 is free — no login required. Try it first, then decide.
            </p>
            {freeSession && (
              <Link
                href={`/learn/session/${freeSession.session}${courseParam}`}
          ...
          <BuyCourse courseId={course.id} courseTitle={course.title} totalSessions={course.totalSessions} />
        </div>
        <div className="mt-10 text-center">
          <Link
            href="/learn"
